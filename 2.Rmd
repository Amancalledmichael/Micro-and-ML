...
# OLS
```{r}
OLS <- lm(hours ~ kidslt6 + kidsge6 + age + educ + city + exper + expersq,
          data = labsupp)
summary(OLS)
```
...
# 1)
```{r}
tobit <- tobit(hours ~ kidslt6 + kidsge6 + age + educ + city + exper + expersq,
               data = labsupp)
summary(tobit)
...
# 2)
```{r}
trunc <- truncreg(hours ~ kidslt6 + kidsge6 + age + educ + city + exper + expersq,
                  data = labsupp)
summary(trunc)
...
# 3)
Wage is highly endogenous to hours. Hours affect wages and vice versa. "bad control" problem. The regression equation can be interpreted as how family care affects labour supply of women of same age, education and experience living in the same 
